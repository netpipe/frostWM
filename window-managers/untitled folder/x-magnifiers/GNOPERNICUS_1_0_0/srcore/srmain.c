...
/* mouse motion idle support */
#define SRC_MOUSE_MOVED_TIMEOUT		300 /* ms */
static gboolean src_mouse_is_moving = FALSE;
static guint mouse_stopped_moving_timeout_handler = 0;
static guint mouse_is_moving_idle_handler = 0;
static gint context_switched = 0;
...
src_present_real_sro_for_brl_and_brlmon (SRObject *obj, gchar *reason)
{
    ...
    if ((src_use_speech || src_use_magnifier) && srl_is_object_watched (obj))
	return;
	...
	if (role == SR_ROLE_TEXT_SL || role == SR_ROLE_TEXT_ML || role == SR_ROLE_TERMINAL 
	    || role == SR_ROLE_COMBO_BOX || role == SR_ROLE_PASSWORD_TEXT)
	{
	    SRLong offset;
...
src_present_real_sro_for_speech (SRObject *obj, gchar *reason)
{
	...
	if (txt && txt[0])
	{
		...
		{
		    if (strcmp (reason, "object:text-changed:insert") == 0 && role == SR_ROLE_TERMINAL)
			src_speech_send_chunk (txt, SRC_SPEECH_PRIORITY_LOW, FALSE);
		    else
...
src_present_real_sro_for_magnifier (SRObject *obj, gchar *reason)
{
    ...
    if (src_use_speech && srl_is_object_watched (obj))
	return;

    if (src_mouse_is_moving)
	return;
...
src_present_crt_window ()
{
	...
	txt = src_presentation_sro_get_pres_for_device (src_crt_window, reason, "speech");
	    ...
	    else if (strcmp (reason, "window:rename") == 0)
		src_speech_send_chunk (txt, SRC_SPEECH_PRIORITY_LOW, TRUE);
	    else if (strcmp (reason, "object:context-switched") == 0)
	    {
		context_switched = 0;
	    	src_speech_send_chunk (txt, SRC_SPEECH_PRIORITY_ERROR, TRUE);
	    }
	    else
		src_speech_send_chunk (txt, SRC_SPEECH_PRIORITY_SYSTEM, TRUE);
	}
...
src_process_config_changed_for_braille (SRConfigStructure *config)
{
    ...
    {
    	src_braille_set_port ();
    }
    else if (strcmp (config->key, BRAILLE_PORT) == 0)
    {
    	src_braille_set_port ();
    }
    else
...
src_process_config_changed_for_magnifier (SRConfigStructure *config)
{
	...
	{
	    gchar *tmp;
	    tmp = g_strdup_printf (crosswire ? _("crosswire on") : _("crosswire off"));
	    src_say_message (tmp);
	    g_free (tmp);
	}
    }
	...
	{
	    gchar *tmp;
	    tmp = g_strdup_printf (crosswire_clip ? _("crosswire clip on") :
				                    _("crosswire clip off"));
	    src_say_message (tmp);
	    g_free (tmp);
	}
    }
	...
	{
	    gchar *tmp;
	    tmp = g_strdup_printf ("%s %d", _("crosswire size"), crosswire_size);
    	    src_say_message (tmp);
	    g_free (tmp);
	}
    }
	...
	{
	    gchar *tmp;
	    tmp = g_strdup_printf ("%s %u", _("crosswire color"), crosswire_color);
    	    src_say_message (tmp);
	    g_free (tmp);
	}
    }
...
src_process_modifier (gchar *modifier)
{
	...
	else if (strncmp (modifier, "Super", 5) == 0)
	{
	    part1 = _("Window");
	    pos = 6;
        }	
	else if (strncmp (modifier, "Hyper", 5) == 0)
	{
	    part1 = _("Menu");
	    pos = 6;
        }	
...
src_process_cursor (gchar *cursor)
{
	...
	if (strcmp (cursor, "Home") == 0)
	    ...
	    src_last_cursor_echo = g_strdup (_("Menu"));
	else
	    sru_assert_not_reached ();
		 ...
		 unsigned long flags)
{
	...
	strncmp (srhotkey_data->keystring, "Super", 5) == 0 ||
	strncmp (srhotkey_data->keystring, "Hyper", 5) == 0)
    {
	...
	src_process_modifier (srhotkey_data->keystring);
	     ...
	     strcmp (srhotkey_data->keystring, "Menu") == 0)
    {
...
static gboolean
src_mouse_moved_timeout (gpointer data)
{
    src_mouse_is_moving = FALSE;
    return FALSE;
}

static gboolean
src_mouse_moved_set_timeout (gpointer data)
{
    if (mouse_stopped_moving_timeout_handler) 
    {
	g_source_remove (mouse_stopped_moving_timeout_handler);
    }
    mouse_stopped_moving_timeout_handler = 
	g_timeout_add (SRC_MOUSE_MOVED_TIMEOUT, src_mouse_moved_timeout, NULL);

    mouse_is_moving_idle_handler = 0;
    return FALSE;
}
		 ...
		 unsigned long flags)
{
    ...
    src_mouse_is_moving = TRUE;
    if (!mouse_is_moving_idle_handler)
    {
	mouse_is_moving_idle_handler = g_idle_add (src_mouse_moved_set_timeout, NULL);
    }
...
static gboolean
src_login_focus (gpointer data)
{
    SRObject *focus;

    focus = srl_get_focus_object ();
    if (focus)
    {
	src_process_sro (focus);
	sro_release_reference (focus);
    }
    return FALSE;
}
...
/*    src_speech_count_mode = DEFAULT_SPEECH_REPEAT_TYPE;*/
    ...
    SET_SRCORE_CONFIG_DATA(SRCORE_INITIALIZATION_END, CFGT_BOOL, &initialize_finished);    
    if (login_time)
	g_idle_add (src_login_focus, NULL);
}
      ...
      gchar **argv)
{
    ...
    if (braille_port < MIN_SERIAL_PORT_NO ||
	braille_port > MAX_SERIAL_PORT_NO)
    {
