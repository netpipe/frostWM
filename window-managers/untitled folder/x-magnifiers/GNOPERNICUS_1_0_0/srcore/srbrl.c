...
src_braille_restart ()
{
    ...
    gchar *port = g_strdup (src_braille->port);
...
static gchar*
src_braille_get_port_from_no (gint port_no)
{
    gchar *port = NULL;

    if (MIN_SERIAL_PORT_NO > port_no ||
	    MAX_SERIAL_PORT_NO < port_no) 
    {
	port_no = DEFAULT_BRAILLE_PORT_NO;
    	SET_BRAILLE_CONFIG_DATA (BRAILLE_PORT_NO, CFGT_INT, &port_no);
    }   
    switch (port_no)
    {
	case -1: port = NULL; 	break;
	case  0: port = g_strdup (TTYNAME0); 	break;
	case  1: port = g_strdup (TTYNAME1);	break;
	case  2: port = g_strdup (TTYNAME2); 	break;
	case  3: port = g_strdup (TTYNAME3);	break;
	default:
	    fprintf (stderr, "\nbrl_open_port: Invalid serial port number %d", port_no);
	    break;
    }
    if (!port && DEFAULT_BRAILLE_PORT)
	port = g_strdup (DEFAULT_BRAILLE_PORT);
...
static gchar*
src_braille_get_port ()
{
    gchar *port = DEFAULT_BRAILLE_PORT;

    GET_BRAILLE_CONFIG_DATA_WITH_DEFAULT (BRAILLE_PORT, CFGT_STRING, 
					      &port, &port);
    if (!port || !port[0])
    {
	gint port_no = DEFAULT_BRAILLE_PORT_NO;
        GET_BRAILLE_CONFIG_DATA_WITH_DEFAULT (BRAILLE_PORT_NO, CFGT_INT, 
					      &port_no, &port_no);
	port = src_braille_get_port_from_no (port_no);
    }
    return port;
}
...
src_braille_load_values (SRCBraille *braille)
{
    ...
    g_free (braille->port);
    braille->port = src_braille_get_port ();
...
src_braille_init ()
{
    ...
    {
	brlval = FALSE;
	fprintf(stderr, "\n** BRAILLE initialization failed for %s on port %s",
					src_braille->device, src_braille->port ? src_braille->port : "<NONE>");
	...
	fprintf (stderr, "\n** BRAILLE initialization succeded for %s device"
				    " on port %s.", src_braille->device, src_braille->port ? src_braille->port : "<NONE>");
...
gboolean
src_braille_change_port (gchar *nport)
{
    SRCBraille *braille;
    ...
    if (!nport || !braille->port || strcmp (braille->port, nport) != 0)
    {
	g_free (braille->port);
	braille->port = nport ? g_strdup (nport) : NULL;
	src_use_braille = src_braille_restart ();
	return src_use_braille;
    }
...
{
    gboolean rv;
    gchar *nport = src_braille_get_port ();

    rv = src_braille_change_port (nport);
    g_free (nport);
...
gboolean
src_braille_set_port_no (gint port_no)
{
    gboolean rv;
    gchar *nport = src_braille_get_port_from_no (port_no);
