...
#define SRU_DEBUG_MODULE "srcspeech"
...
static guint      speech_timeout_id;
static gboolean   src_speech_restored = FALSE;
...
static GPtrArray	*src_created_voices = NULL;
...
src_speech_valid_voice (gchar *voice)
{
    ...
    for (i = 0; i < src_created_voices->len; i++)
    {
	if (strcmp (voice, g_ptr_array_index (src_created_voices, i)) == 0)
	    return TRUE;
    }
    return FALSE;
}
...
src_speech_send (gchar *spcoutput)
{
    ...
    sru_debug_log ("out", "send to speech:%s", spcoutput);
...
src_speech_add_voice (gchar *voice)
{
    ...
    src_speech_get_voice_parameters_from_gconf (voice);
    g_free (voices);
...
static gboolean
src_timeout_start_speech (gpointer data)
{
    gboolean use_speech = TRUE;
    SET_SRCORE_CONFIG_DATA (SRCORE_SPEECH_ACTIVE, CFGT_BOOL, &use_speech);
    speech_timeout_id = 0;
    src_speech_restored = TRUE;
    return FALSE;    
}

#define SRC_SPEECH_RESTART_TIMEOUT 1000
static void
src_restart_speech ()
{
    g_printerr ("\nRestarting speech.\n");
...
src_speech_init ()
{
    ...
    SRU_DEBUG_ADD_MODULE_AND_SCOPES;

    src_speech_idle = TRUE;
    srcs_outs = NULL;
    last_out = NULL;
    src_created_voices = g_ptr_array_new ();
    src_voice_params = g_ptr_array_new ();
    ...
    srcs_in_init ();
    terminate = src_speech_initialized = srs_init (srcs_in_parse_input, src_restart_speech);
#ifdef SRS_NO_MARKERS_SUPPORTED
	...
	{	
	    ...
	    for (i = 0; i < src_created_voices->len; i++)
	    {
		if (strcmp ("default", g_ptr_array_index (src_created_voices, i)) == 0)
		{
		    src_speech_initialized = TRUE;
		    break;
		}
	    }
	}
    ...
    if (!src_speech_initialized && terminate)
	srs_terminate ();
	...
	if (src_speech_restored)
	    src_speech_say_message (_("Speech restored"));
    }
    else
    {
...
src_speech_translate_symbol (gint index)
{
	...
	if (src_speech_dictionary[index].character == '.' ||
	    ...
	    src_speech_dictionary[index].character == '?')
	{
	    rv = g_strconcat ("  ", _(src_speech_dictionary[index].singular), " ", NULL);
			 ...
			 gchar **new_pos)
{
	...
	rv = g_strconcat (count, " ",
	    cnt == 1 ? _(src_speech_dictionary[index].singular) : 
					_(src_speech_dictionary[index].plural),
	    " ", NULL);
    *new_pos = crt;
    return rv;
}
			   ...
			   SRCSpeechSpellingMode smode)
{
	...
	if (index != -1)
	    return g_strdup (_(src_speech_dictionary[index].singular));
	else
			    ...
			    )
{
    ...
    g_ptr_array_add (src_created_voices, g_strdup (name));
#ifdef SRS_NO_MARKERS_SUPPORTED
    if (src_speech_callback)
    {
