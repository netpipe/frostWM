...
typedef enum
{
    ...
    SRC_PRESENTATION_STATE_SELECTED,
...
src_presentation_get_item_type (const gchar *item)
{
    ...
    }src_presentation_type_name [] = 
	{
	    ...
	    {SRC_PRESENTATION_STATE_SELECTED,	"state:selected"},
				    ...
				    gpointer info)
{
	...
	*((gchar**)info) = NULL;
	...
	case SRC_PRESENTATION_STATE_CHECKED:
	    ...
	    break;
	case SRC_PRESENTATION_STATE_SELECTED:
	    {
		SRState state;
		if (sro_get_state (obj, &state, SR_INDEX_CONTAINER))
		    rv = state & SR_STATE_SELECTED ? TRUE : FALSE;
	    }
	    break;
					    ...
					    const gchar *reason)
{
    ...
    gint default_int_val = 0;
    
    static gint size_x = 0,
	        size_y = 0;
	...
	switch (btype)
	{
		...
		if ((location.width - location.x)  >= size_x + 1 &&
    		    (location.height - location.y) >= size_y + 1)
			get = FALSE;
		break;
