...
brlconf_load_default_settings (Braille* braille)
{
    ...
    value = gconf_client_get_default_from_schema (gnopernicus_client, 
						CONFIG_PATH BRAILLE_KEY_PATH BRAILLE_PORT, 
						NULL);
    if (value)
    {
    	braille->port = g_strdup (gconf_value_get_string (value));
	gconf_value_free (value);
	value = NULL;
    }
    else
    {
	if (DEFAULT_BRAILLE_PORT)
	    braille->port = g_strdup (DEFAULT_BRAILLE_PORT);
	else
	    braille->port = NULL;
    }
...
void
brlconf_port_set (const gchar *port)
{
    if (!gnopiconf_set_string (port, CONFIG_PATH BRAILLE_KEY_PATH BRAILLE_PORT))
    {
        sru_warning (_("Failed to set serial port : %s."), port);
    }
}
...
gchar* 
brlconf_port_get (void)
{
    return gnopiconf_get_string_with_default (CONFIG_PATH BRAILLE_KEY_PATH BRAILLE_PORT, 
					DEFAULT_BRAILLE_PORT);
}
...
{
    return DEFAULT_BRAILLE_DEVICE_INDEX;
}
