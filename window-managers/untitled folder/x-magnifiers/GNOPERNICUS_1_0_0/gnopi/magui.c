...
static void
magui_set_zoomfactor_lock_on_off ()
{
...
static void
magui_zoomfactor_lock_toggled (GtkToggleButton *button, gpointer user_data)
{
    if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (ck_lock_factor)))
    {
	gtk_spin_button_set_value   (GTK_SPIN_BUTTON (sp_factorx), 
				     magnifier_setting->zoomfactorx);
...
static void
magui_zoomfactor_x_value_changed (GtkSpinButton *spin, gpointer user_data)
{
    gdouble yval = gtk_spin_button_get_value (GTK_SPIN_BUTTON (sp_factory));
    if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (ck_lock_factor)))
    {
	gdouble xval = gtk_spin_button_get_value (spin);
	if (xval != yval)
	    gtk_spin_button_set_value (GTK_SPIN_BUTTON (sp_factory), xval);
    }    
}

static void
magui_zoomfactor_y_value_changed (GtkSpinButton *spin, gpointer user_data)
{
    gdouble xval = gtk_spin_button_get_value (GTK_SPIN_BUTTON (sp_factorx));
    if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (ck_lock_factor)))
    {
	gdouble yval = gtk_spin_button_get_value (spin);
	if (xval != yval)
	    gtk_spin_button_set_value (GTK_SPIN_BUTTON (sp_factorx), yval);
    }
}

static void
magui_set_focus_tracking_mode ()
{
...
{    
    ...
    glade_xml_signal_connect (xml, "on_sp_zoomfactor_x_value_changed",
			      GTK_SIGNAL_FUNC (magui_zoomfactor_x_value_changed));
			      
    glade_xml_signal_connect (xml, "on_sp_zoomfactor_y_value_changed",
			      GTK_SIGNAL_FUNC (magui_zoomfactor_y_value_changed));
    glade_xml_signal_connect (xml, "on_ck_lock_factor_toggled",
			      GTK_SIGNAL_FUNC (magui_zoomfactor_lock_toggled));
}
