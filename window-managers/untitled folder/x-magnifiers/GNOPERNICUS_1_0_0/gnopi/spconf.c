...
gboolean 
spconf_key_is_valid (gchar *path)
{
    return gconf_valid_key (path, NULL);
}
...
const gchar*
spconf_voice_list_get_default_voice (GSList *voices, 
				     gchar *voice_name)
{
    GSList *tmp;
    
    for (tmp = voices; tmp; tmp = tmp->next)
    {
	SpeechVoice *speech_voice = (SpeechVoice *)tmp->data;
	if (!strcmp (speech_voice->voice_name, voice_name))
	    return speech_voice->voice_name_descr;
    }

    return (((SpeechVoice *) voices->data)->voice_name_descr);
}

const SpeechDriver*
spconf_driver_list_get_default_driver (SpeechDriverListType *driver_list,
				       const gchar *driver_name)
{
    SpeechDriverListType *tmp;
    ...
    return ((SpeechDriver *) driver_list->data);
}
