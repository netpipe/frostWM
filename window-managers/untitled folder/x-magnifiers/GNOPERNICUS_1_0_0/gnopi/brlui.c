...
static GtkWidget 	*cb_port;
static GtkWidget	*lb_port;
static GtkWidget	*ck_man_port;
static GtkWidget 	*et_man_port;
/**
...
brlui_braille_device_changeing (Braille *braille_setting)
{
    ...
    if (gtk_combo_box_get_active_iter (GTK_COMBO_BOX (cb_port), &iter))
    {
	gchar *str, *error;
	gtk_tree_model_get (gtk_combo_box_get_model (GTK_COMBO_BOX (cb_port)),
			    &iter, 0, &str, -1);
	braille_setting->port_no = strtol (str, &error, 10);
	if ((error && *error) || braille_setting->port_no < MIN_SERIAL_PORT_NO ||
		braille_setting->port_no > MAX_SERIAL_PORT_NO)
	    braille_setting->port_no = -1;
	g_free (str);
    }
    else
        braille_setting->port_no = -1;
    brlconf_port_no_set (braille_setting->port_no);
    brlconf_port_set (braille_setting->port);
}
				    ...
				    gpointer  user_data)
{
    ...
    gboolean	active_port_ck, active_port_et, active_port_no;
    sru_return_if_fail (braille_setting);    
    ...
    active_port_ck = active_port_et = active_port_no = FALSE;
    if (sensitivity)
    {
	active_port_ck = TRUE;
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (ck_man_port)))
	    active_port_et = TRUE;
	else
	    active_port_no = TRUE;
    }

    gtk_widget_set_sensitive (GTK_WIDGET (lb_port), active_port_no);
    gtk_widget_set_sensitive (GTK_WIDGET (cb_port), active_port_no);
    gtk_widget_set_sensitive (GTK_WIDGET (et_man_port), active_port_et);
    gtk_widget_set_sensitive (GTK_WIDGET (ck_man_port), active_port_ck);
...
static void
brlui_braille_device_ck_man_port_toggled (GtkToggleButton *button, 
					  gpointer user_data)
{
	...
	gtk_widget_set_sensitive (et_man_port, state);
	gtk_widget_set_sensitive (lb_port, !state);
	gtk_widget_set_sensitive (cb_port, !state);
	if (!state)
	{
	    g_free (braille_setting->port);
	    braille_setting->port = g_strdup ("");
	    gtk_entry_set_text (GTK_ENTRY (et_man_port), "");
	}
    }
}
static gboolean
brlui_braille_device_et_man_port_changed (GtkWidget *widget,
                                            GdkEventFocus *event,
                                            gpointer user_data)
{
    g_free (braille_setting->port);
    braille_setting->port = g_strdup (gtk_entry_get_text (GTK_ENTRY (et_man_port)));
				   ...
				   Braille *braille_setting)
{
    ...
    cb_port 	      = glade_xml_get_widget (xml, "cb_port");
    lb_port 	      = glade_xml_get_widget (xml, "lb_port");
    ck_man_port	      = glade_xml_get_widget (xml, "ck_man_port");
    et_man_port	      = glade_xml_get_widget (xml, "et_man_port");
    ...
    glade_xml_signal_connect (xml,"on_ck_man_port_toggled",		
			    GTK_SIGNAL_FUNC (brlui_braille_device_ck_man_port_toggled));

    glade_xml_signal_connect (xml,"on_et_man_port_focus_out_event",		
			    GTK_SIGNAL_FUNC (brlui_braille_device_et_man_port_changed));
...
brlui_load_braille_device (Braille *braille_setting)
{
	...
	gtk_window_set_destroy_with_parent (GTK_WINDOW (w_braille_device), 
						TRUE);
	gtk_combo_box_append_text (GTK_COMBO_BOX (cb_port), _("<none>"));
	for (i = 0; i <= MAX_SERIAL_PORT_NO; i++)
	{
	    gchar *port;
	    port = g_strdup_printf ("%d", i);
	    gtk_combo_box_append_text (GTK_COMBO_BOX (cb_port), port);
	    g_free (port);
	}
    }
    else
...
{    
    GtkTreeModel* model;
    GtkTreeIter iter, iter_found;
    gboolean found = FALSE, hasnone = FALSE;
    gint port;
    gchar *str, *error;
    const gchar *dev;
    ...
    dev = brlconf_return_active_device ();
    if (!dev)
	dev = _("<none>");
    gtk_entry_set_text (GTK_ENTRY (GTK_COMBO (cb_braille_device)->entry), 
			dev);

    model = gtk_combo_box_get_model (GTK_COMBO_BOX (cb_port));
    if (!model)
	return FALSE;
	...
	if (strcmp (str, _("<none>")) == 0 && !found)
	{
	    iter_found = iter;
	    hasnone= TRUE;
	}
	port = strtol (str, &error, 10);
	if ((!error || !*error) && port == braille_setting->port_no)
	{
	    sru_assert (found == FALSE);
	    iter_found = iter;
	    found = TRUE;
	}
	g_free (str);
    ...
    if (found || hasnone)
	gtk_combo_box_set_active_iter (GTK_COMBO_BOX (cb_port), &iter_found);
    gtk_entry_set_text (GTK_ENTRY (et_man_port), braille_setting->port);
    if (braille_setting->port && braille_setting->port[0])
    {
	gtk_widget_set_sensitive (GTK_WIDGET (lb_port), FALSE);
	gtk_widget_set_sensitive (GTK_WIDGET (cb_port), FALSE);
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (ck_man_port), TRUE);
    }
