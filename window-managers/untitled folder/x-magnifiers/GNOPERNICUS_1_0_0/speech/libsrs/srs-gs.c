...
srs_gs_speaker_new (SRSVoiceInfo *voice)
{
    ...
    if (!srs_gs_speaker_update (speaker, voice))
    {
	srs_gs_speaker_terminate (speaker);
	speaker = NULL;
    }
...
gboolean
srs_gs_init (SRSGSMarkersCallback callback, SRSResetCallback reset_callback)
{
    sru_assert (callback);
    ...
    return srs_gs_wrap_init (srs_gs_callback, reset_callback);
}
