...
#define SRL_MAX_CHILDREN_CNT		1000
#define	SRL_MAX_CLIENTS			1
#define SRL_MAX_CHILDREN_CNT 		200	/* depth of child searches */
#define	SRL_LOG
/* #define SRL_TIMING */
...
static Accessible		*srl_last_at_spi_focus	= NULL;
...
static Accessible*
srl_get_focused_cell (Accessible *acc_table)
{
    Accessible *rv = NULL;
    AccessibleTable *table;
    AccessibleComponent *comp;
    gint rows, rowe, cols, cole;

    srl_assert (acc_table);

    table = Accessible_getTable (acc_table);
    comp = Accessible_getComponent (acc_table);
	...
	AccessibleComponent_getExtents  (comp, &x, &y, &w, &h,
						    SPI_COORD_TYPE_WINDOW);
	/* get the uper-left corner */
    	corner = AccessibleComponent_getAccessibleAtPoint (comp, x + 1, y + 1,
					    	    SPI_COORD_TYPE_WINDOW);
	if (corner)
	{
	    index = Accessible_getIndexInParent (corner);
	    rows = AccessibleTable_getRowAtIndex (table, index);
	    cols = AccessibleTable_getColumnAtIndex (table, index);
	    Accessible_unref (corner);
	}
	else
	    rows = cols = 0;
	
	/* get the bottom-left corner */
    	corner = AccessibleComponent_getAccessibleAtPoint (comp, x + 1, y + h - 1,
					    	    SPI_COORD_TYPE_WINDOW);
	if (corner)
	{
	    index = Accessible_getIndexInParent (corner);
	    rowe = AccessibleTable_getRowAtIndex (table, index);
	    Accessible_unref (corner);
	}
	else
	    rowe = AccessibleTable_getNRows (table);

	/* get the upper-right corner */
    	corner = AccessibleComponent_getAccessibleAtPoint (comp, x + w - 1, y + 1,
					    	    SPI_COORD_TYPE_WINDOW);
	if (corner)
	{
	    index = Accessible_getIndexInParent (corner);
	    cole = AccessibleTable_getColumnAtIndex (table, index);
	    Accessible_unref (corner);
	}
	else
	    cole = AccessibleTable_getNColumns (table);
	
    }

    if (rows != -1 && cols != -1)
    {
	/* limit our search to at most SRL_MAX_CHILDREN_CNT cells, in */
	/* case we get pathological start/end row/col (or have a large */
	/* screen with a tiny spreadsheet with a gazillion showing cells */
	gint i, j;
	gint count = 0;
...
/* function returns the active child in a tree. The search
   is limited to maximum _limit_ number of children, children's chidren etc */
static Accessible*
srl_tree_get_active_item (Accessible *tree,
			  gint *limit)
{
    Accessible *active = NULL;
    gint cnt, i;

    srl_assert (tree);

    cnt = Accessible_getChildCount (tree);
    for (i = 0; i < cnt; i++)
    {
	Accessible *child;
	child = Accessible_getChildAtIndex (tree, i);
	if (!child)
	    continue;
	(*limit)--;
	if (srl_acc_has_state (child, SPI_STATE_ACTIVE))
	{
	    active = child;
	    Accessible_ref (active);
	}
	else if (srl_acc_has_state (child, SPI_STATE_EXPANDED))
	    active = srl_tree_get_active_item (child, limit);
...
srle_set_acc (SRLEvent *event)
{
	...
	{
	    gint i, cnt;
	    gboolean is_focused_child = FALSE;
	    ...
	    if (!is_focused_child && srle_acc_has_role (event, SPI_ROLE_TABLE))
	    {
		for (i = 0; i < cnt; i++)
		{
		    Accessible *child;
			...
			if (Accessible_getRole (child) == SPI_ROLE_TABLE_CELL &&
			    srl_acc_has_state (child, SPI_STATE_SHOWING) &&
			    srl_acc_has_state (child, SPI_STATE_VISIBLE))
			{
			    event->acc = child;
			    break;
			}
		...
		if (!event->acc)
		    event->acc = srl_get_focused_cell (event->acc_ev->source);
	    }
	    ...
	    AccessibleSelection_unref (selection);
	     ...
	     srle_acc_has_role (event, SPI_ROLE_TREE))
    {
	gint limit = SRL_MAX_CHILDREN_CNT;
	event->acc = srl_tree_get_active_item ((Accessible*)srle_get_acc (event), 
						 &limit);
    }
...
srl_table_same_line (Accessible *acc)
{
	...
	if (table)
	{
	    ...
	    if (row == srl_last_row && parent == srl_last_table)
	        rv = TRUE;
	    srl_last_row = row;
	    AccessibleTable_unref (table);
	}
			...
			SREventType type)
{
	...
	if (role == SR_ROLE_TABLE_COLUMNS_HEADER)
	    srl_last_row = -1;
				 ...
				 SREventType type)
{
    ...
    for (i = index + 1; i < SRL_EVENT_PRIORITY_LAST; i++)
	if (srl_last_events[i] != NULL)
	    return TRUE;
...
srl_get_context (Accessible *acc)
{
    ...
    context = NULL;
    Accessible_ref (acc);
    acc_role = Accessible_getRole (acc);
    if (acc_role == SPI_ROLE_TABLE)
    {
	context = acc;
	...
	if (parent)
	{
	    ...
	    if (role == SPI_ROLE_WINDOW && acc_role == SPI_ROLE_STATUS_BAR)
	    {
		context = acc;
		Accessible_ref (context);
	    }
...
static gboolean
srl_set_checked_info (Accessible *acc,
		      SRLLastInfo *info)
{
    srl_assert (acc && info);

    info->checked = srl_acc_has_state (acc, SPI_STATE_CHECKED);
...
static gboolean srl_set_last_focus (Accessible *acc);
...
srl_event_is_reported (SRLEvent *event)
{
	...
	else if (srle_has_type (event, SRL_EVENT_WINDOW_ACTIVATE) ||
	    srle_has_type (event, SRL_EVENT_WINDOW_RESTORE))
	{
	...
	rv = srle_acc_has_state (event, SPI_STATE_FOCUSED) ||
	     ...
	     srle_acc_has_state (event, SPI_STATE_ACTIVE);
    else if (srle_has_type (event, SRL_EVENT_FOCUS1))
	rv = TRUE;
    else if (srle_has_type (event, SRL_EVENT_FOCUS2))
	rv = TRUE;
    else if (srle_has_type (event, SRL_EVENT_STATE_EXPANDED))
    {
	rv = srle_is_for_focused_ancestor_acc (event);
	if (rv)
	    srl_set_last_focus ((Accessible*) srle_get_acc (event));
	     ...
	     srle_is_for_watched_acc (event))
    {
	if (srle_has_type (event, SRL_EVENT_STATE_CHECKED))
	{
	    if (srle_acc_has_role (event, SPI_ROLE_RADIO_BUTTON))
		rv = !last_info->checked && srle_acc_has_state (event, SPI_STATE_CHECKED);
	    else
...
/*	else if (srle_has_type (event, SRL_EVENT_STATE_SELECTED))
	    ...
	    rv = srl_last_table != NULL; /* selection changed while in a table */
    }
...
static gboolean
srl_set_last_at_spi_focus (Accessible *acc)
{
    if (srl_last_at_spi_focus)
	Accessible_unref (srl_last_at_spi_focus);
    srl_last_at_spi_focus = acc;
    if (srl_last_at_spi_focus)
	Accessible_ref (srl_last_at_spi_focus);
    return TRUE;
}
...
static Accessible*
srl_get_focused_from_acc (Accessible *acc)
{
    Accessible *focused = NULL;
    gint cnt, i;
	...
	{
	    focused = child;
	    Accessible_ref (focused);
	}
	else
	    focused = srl_get_focused_from_acc (child);
...
static Accessible*
srl_get_focused_from_active_toplevel (Accessible *window)
{
    return srl_get_focused_from_acc (window);
}

static Accessible*
srl_get_active_toplevel_from_app (Accessible *app)
{
    Accessible *active = NULL;
    gint cnt, i;
    ...
    cnt = Accessible_getChildCount (app);
    for (i = 0; i < cnt; i++)
    {
	Accessible *child;
	child = Accessible_getChildAtIndex (app, i);
	if (!child)
	    continue;
	if (srl_acc_has_state (child, SPI_STATE_ACTIVE))
	{
...
static Accessible*
srl_get_focused_from_desktop ()
{
    Accessible *desktop, *focused = NULL;
    gint cnt, i;
    ...
    for (i = 0; i < cnt; i++)
    {
	Accessible *child = NULL, *active = NULL;
	child = Accessible_getChildAtIndex (desktop, i);
	if (child)
	    active = srl_get_active_toplevel_from_app (child);
	if (active)
	    focused = srl_get_focused_from_active_toplevel (active);
	if (child)
	    Accessible_unref (child);
	if (active)
	    Accessible_unref (active);
	if (focused)
	    break;
    }
...
SRObject*
srl_get_focus_object ()
{
    SRObject *sro = NULL;
    Accessible *focused;

    focused = srl_get_focused_from_desktop ();
    if (focused)
    {
	srl_last_focus = focused;
	srl_set_info (focused, &last_info_focus);
	if (!sro_get_from_acc (focused, SRL_EVENT_NAME_FOCUS, &sro))
	    sro = NULL;
    }
