	...
	case SR_ROLE_TABLE_LINE:
            {
                ...
                else
                {
                    cell = acc;
                    Accessible_ref (cell);
                    get_text = Accessible_getRole (cell) == SPI_ROLE_LABEL &&
                            Accessible_isText (cell);
                }

		cellname = Accessible_getName (cell);

		/* if this table cell/line implements AccessibleText, use that & the name... */
                if (get_text)
                {
                    AccessibleText *text = Accessible_getText (cell);
                    if (text)
                    {
                        celltext = AccessibleText_getText (text, 0, -1);

			/* ...but only if text & name are different; else use only one */
                        if (cellname && cellname[0] && celltext && celltext[0] && strcmp(cellname, celltext))
			{
                            tmp = g_strconcat (cellname, " ", celltext, NULL);
			    name_ = SR_strdup (tmp);
			    g_free (tmp);
			}
			else if (celltext && celltext[0])
			    name_ = SR_strdup (celltext);
                        else if (cellname && cellname[0])
                            name_ = SR_strdup (cellname);
			SPI_freeString (cellname);
			SPI_freeString (celltext);
                        AccessibleText_unref (text);
                    }
                }
                else if (cellname)
                    name_ = SR_strdup (cellname);	/* no AccText, so just use the name */
	...
	*cell = NULL;
	...
	default:
	{
		...
		if (rowno >= 0 && colcnt > 0)
		{
		    ...
		    {
			glong focus_index = -1;
			gboolean is_focused_cell = FALSE;
			for (i = 0; i < colcnt; i++)
			{
...
/*FIXME: for tree item objects */
	case SPI_ROLE_LABEL:
		...
		while (parent)
		{
		    ...
		    if (Accessible_getRole (parent) == SPI_ROLE_TREE)
			role = SR_ROLE_TREE_ITEM;
		    else if (Accessible_getRole (parent) == SPI_ROLE_LIST)
			role = SR_ROLE_LIST_ITEM;
		    else if (Accessible_getRole (parent) == SPI_ROLE_TABLE)
			role = SR_ROLE_TABLE_CELL;
			
		    if (role != SR_ROLE_LABEL)
			break;
		}
		if (parent)
		    Accessible_unref (parent);
			...
			SRLong index)
{
    ...
    /* Read the whole line when navigating from a single-word line */						
    if (start_last_word == start_last_line && end_last_word == end_last_line)
	return FALSE;
			     ...
			     SRLong index) 
{
    ...
    long int x, y, w, h, offset,
	      ...
	      rng_x, rng_y, rng_w, rng_h, rng_start_offset, rng_end_offset;
    ...
    if (offset == AccessibleText_getCharacterCount (acc_text) && offset > 0)
	{
		...
		if (offset == line_start_offset && offset > 0)
			first_in_line = TRUE;/*our char is the first char in the line*/
	...
	if (last)
	{
		...
		/*ofset+1 is guaranteed since this is last char in text*/
		rng_end_offset = offset+1;
		if (first_in_line)
		{
			/*making sure that the range starts on the same line as the caret*/
			rng_start_offset = offset+1;
			rng_end_offset = offset+2;
		}
	}
	else
	{
		/*ofset-1 is guaranteed since this is NOT last char in text*/
		rng_start_offset = offset-1;
		rng_end_offset = offset;
	}
	
	AccessibleText_getRangeExtents (acc_text, rng_start_offset, rng_end_offset, 
								    &rng_x, &rng_y, &rng_w, &rng_h, 
									acc_type);	
	
	AccessibleText_getCharacterExtents (acc_text, offset, 
										&x, &y, &w, &h, acc_type);
    
	location->width = rng_w;
	if (last)
	{		
		location->x = rng_x;
		/*adding the width of the previus character*/
		location->width += w;	
	}
	else
	{
...
gchar*
sro_get_text_with_flows_relation (Accessible *acc)
{
    ...
    gint k;
    long int current_x, current_y, current_width, current_height;
    SRLong offset, start, end;
    
    GString *final_line = g_string_new ("");
    
    comp = Accessible_getComponent (acc);	
    srl_return_val_if_fail (comp, FALSE);
    AccessibleComponent_getExtents (comp, &current_x, &current_y, &current_width, &current_height, SPI_COORD_TYPE_SCREEN);
    AccessibleComponent_unref (comp);

    acc_text = get_text_from_acc (acc);
    srl_return_val_if_fail (acc_text, FALSE);
    offset = AccessibleText_getCaretOffset (acc_text);
    get_text_range_from_offset (acc_text, SR_TEXT_BOUNDARY_LINE, offset, &start, &end);
    fl = g_new (FLWLine, 1);    	
    fl->x = current_x;
    fl->text_line = AccessibleText_getText (acc_text, start, end);
    AccessibleText_unref (acc_text);
    
    lines = g_array_new (FALSE, FALSE, sizeof (FLWLine*));	
    g_array_append_val (lines, fl);
    
    rel = Accessible_getRelationSet (acc);
    if (rel)
    {		    
	for (k = 0; rel[k]; k++)
	{
	    AccessibleRelation *crt;
	    crt = rel[k];
	    if (SPI_RELATION_FLOWS_TO == AccessibleRelation_getRelationType (crt) ||
		SPI_RELATION_FLOWS_FROM == AccessibleRelation_getRelationType (crt))
	    {
		gint cnt, i;
		cnt = AccessibleRelation_getNTargets (crt);
		for (i = 0; i< cnt; i++)
		{
		    Accessible *acc;
		    ...
		    acc = AccessibleRelation_getTarget (crt, i);
		    if (acc)
			comp = Accessible_getComponent (acc);
		    if (acc)
		    ...
		    if (comp && acc_text)
		    {
			AccessibleComponent_getExtents (comp, &x, &y, &width, &height, SPI_COORD_TYPE_SCREEN);
		    
			if ((y == current_y) ||
		    	    (current_y > y && (current_y - height) < y) ||
			    (current_y < y && (current_y + current_height) > y))
			{
			    FLWLine *f = g_new (FLWLine, 1);
			    f->x = x;
			    f->text_line = AccessibleText_getText (acc_text, 0, -1);
			    for (j = 0; j < lines->len; j++)
			    {
				FLWLine *l = g_array_index (lines, FLWLine*, j);
				if (l->x >= f->x)
				{
				    g_array_insert_val (lines, j, f);
				    break;
				}
			    }
			    if (j == lines->len)
    ...
    {
	FLWLine *l = g_array_index (lines, FLWLine*, k);
	if (k > 0)
	    final_line = g_string_append (final_line, " ");
	final_line = g_string_append (final_line, l->text_line);
    }
    
    for (k = 0; k < lines->len; k++)
    {
	FLWLine *l = g_array_index (lines, FLWLine*, k);
	SPI_freeString (l->text_line);
	g_free (l);
    }
    g_array_free (lines, TRUE);

    return g_string_free (final_line, FALSE);
}
			      ...
			      SRLong index)
{
	...
	*text = NULL;
    ...
    if (type == SR_TEXT_BOUNDARY_LINE)
	tmp =  sro_get_text_with_flows_relation (acc);
    else
		       ...
		       gchar *prev)
{
	...
	Accessible_getRole (acc) != SPI_ROLE_TOGGLE_BUTTON)
    {
