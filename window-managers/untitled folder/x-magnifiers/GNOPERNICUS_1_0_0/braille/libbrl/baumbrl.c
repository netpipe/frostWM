...
baum_brl_input_parser (gint new_val)
{
	...
	case BIPS_EXP_KEYS:		
		...
		switch (baum_dev_type)
		{
		    ...
		    case BAUMDT_VARIO20:
		    case BAUMDT_PRONTO:
		    case BAUMDT_SUPERVARIO40:
		    case BAUMDT_POCKETVARIO24:	
			/* direct mapping, device has no other keys */
		      ...
		      BRLDevice      *device)
{
    ...
    else if (strcmp ("PRONTO", device_name) == 0) 	
    {
	device->cell_count = 18;
	device->display_count = 1;
	device->displays[0].start_cell = 0;
	device->displays[0].width = 18;
	device->displays[0].type = BRL_DISP_MAIN;
	device->input_type = BRL_INP_BITS;
	device->key_count = 6;
	
	baumdd.hms_length = 5;	/* ??? */
	baum_dev_type = BAUMDT_PRONTO;

	rv = 1;
    }
    
    else if (strcmp ("POCKETVARIO24", device_name) == 0) 	
    {
	device->cell_count = 24;
	device->display_count = 1;
	device->displays[0].start_cell = 0;
	device->displays[0].width = 24;
	device->displays[0].type = BRL_DISP_MAIN;
	device->input_type = BRL_INP_BITS;
	device->key_count = 6;
	
	baumdd.hms_length = 5;	/* ??? */
	baum_dev_type = BAUMDT_POCKETVARIO24;

	rv = 1;
    }
    else if (strcmp ("SUPERVARIO40", device_name) == 0) 	
    {
	device->cell_count = 40;
	device->display_count = 1;
	device->displays[0].start_cell = 0;
	...
	baumdd.hms_length = 5;	/* ??? */
	baum_dev_type = BAUMDT_SUPERVARIO40;
